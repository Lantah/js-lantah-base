describe('Memo.constructor()', function () {
  it('throws error when type is invalid', function () {
    expect(() => new LantahBase.Memo('test')).to.throw(/Invalid memo type/);
  });
});

describe('Memo.none()', function () {
  it('converts to/from xdr object', function () {
    let memo = LantahBase.Memo.none().toXDRObject();
    expect(memo.value()).to.be.undefined;
    let baseMemo = LantahBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(LantahBase.MemoNone);
...
describe('Memo.text()', function () {
  it('returns a value for a correct argument', function () {
    expect(() => LantahBase.Memo.text('test')).to.not.throw();
    let memoUtf8 = LantahBase.Memo.text('三代之時');
  ...
  it('returns a value for a correct argument (utf8)', function () {
    let memoText = LantahBase.Memo.text([0xd1]).toXDRObject().toXDR();
    ...
    memoText = LantahBase.Memo.text(Buffer.from([0xd1]))
      .toXDRObject()
      .toXDR();
    expect(memoText.equals(expected)).to.be.true;
  });
  ...
  it('converts to/from xdr object (buffer)', function () {
    ...
    let memo = LantahBase.Memo.text(buf).toXDRObject();
    expect(memo.arm()).to.equal('text');
  ...
  it('throws an error when invalid argument was passed', function () {
    expect(() => LantahBase.Memo.text()).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
    expect(() => LantahBase.Memo.text({})).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
    expect(() => LantahBase.Memo.text(10)).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
    expect(() => LantahBase.Memo.text(Infinity)).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
    expect(() => LantahBase.Memo.text(NaN)).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
  });
  ...
  it('throws an error when string is longer than 28 bytes', function () {
    expect(() =>
      LantahBase.Memo.text('12345678901234567890123456789')
    ).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => LantahBase.Memo.text('三代之時三代之時三代之時')).to.throw(
      /Expects string, array or buffer, max 28 bytes/
    );
  });
});
...
describe('Memo.id()', function () {
  it('returns a value for a correct argument', function () {
    expect(() => LantahBase.Memo.id('1000')).to.not.throw();
    expect(() => LantahBase.Memo.id('0')).to.not.throw();
  });

  it('converts to/from xdr object', function () {
    let memo = LantahBase.Memo.id('1000').toXDRObject();
    expect(memo.arm()).to.equal('id');
    expect(memo.id().toString()).to.equal('1000');

    let baseMemo = LantahBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(LantahBase.MemoID);
    expect(baseMemo.value).to.be.equal('1000');
  });

  it('throws an error when invalid argument was passed', function () {
    expect(() => LantahBase.Memo.id()).to.throw(/Expects a int64/);
    expect(() => LantahBase.Memo.id({})).to.throw(/Expects a int64/);
    expect(() => LantahBase.Memo.id(Infinity)).to.throw(/Expects a int64/);
    expect(() => LantahBase.Memo.id(NaN)).to.throw(/Expects a int64/);
    expect(() => LantahBase.Memo.id('test')).to.throw(/Expects a int64/);
  });
});
...
describe('Memo.hash() & Memo.return()', function () {
  ...
  it('return converts to/from xdr object', function () {
    ...
    // Testing string hash
    let memo = LantahBase.Memo.return(buffer.toString('hex')).toXDRObject();
    ...
    let baseMemo = LantahBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(LantahBase.MemoReturn);
    expect(Buffer.isBuffer(baseMemo.value)).to.be.true;
    expect(baseMemo.value.length).to.equal(32);
    expect(baseMemo.value.toString('hex')).to.be.equal(buffer.toString('hex'));
  });

  var methods = [LantahBase.Memo.hash, LantahBase.Memo.return];
