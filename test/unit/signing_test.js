...
describe('LantahBase#sign', function () {
  let expectedSig =
  ...
  it('can sign an string properly', function () {
    ...
    let actualSig = LantahBase.sign(data, secretKey).toString('hex');
...
describe('LantahBase#verify', function () {
  let sig = Buffer.from(
    '587d4b472eeef7d07aafcd0b049640b0bb3f39784118c2e2b73a04fa2f64c9c538b4b2d0f5335e968a480021fdc23e98c0ddf424cb15d8131df8cb6c4bb58309',
    'hex'
  );
  ...
  it('can verify an string properly', function () {
    let data = 'hello world';
    expect(LantahBase.verify(data, sig, publicKey)).to.be.ok;
    expect(LantahBase.verify('corrupted', sig, publicKey)).to.not.be.ok;
    expect(LantahBase.verify(data, badSig, publicKey)).to.not.be.ok;
  });
