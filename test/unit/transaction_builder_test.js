...
const { encodeMuxedAccountToAddress } = LantahBase;
...
describe('TransactionBuilder', function () {
  describe('constructs a native payment transaction with one operation', function () {
    ...
    beforeEach(function () {
      ...
      memo = LantahBase.Memo.id('100');

      transaction = new LantahBase.TransactionBuilder(source, {
        fee: 100,
    ...
    it('should have 100 ug fee', function (done) {
  ...
  describe('constructs a transaction with soroban data', function () {
    ...
    beforeEach(function () {
      ...
      sorobanTransactionData = new LantahBase.SorobanDataBuilder()
        .setResources(0, 5, 0)
        .setRefundableFee(1)
        .build();
    });
    ...
    it('should set the soroban data from object', function (done) {
      let transaction = new LantahBase.TransactionBuilder(source, { fee: 100 })
        .setNetworkPassphrase(LantahBase.Networks.TESTNET)
        .addOperation(
  ...
  describe('constructs a native payment transaction with two operations', function () {
    ...
    it('should have 200 ug fee', function (done) {
  ...
  describe('constructs a native payment transaction with custom base fee', function () {
    ...
    it('should have 2000 ug fee', function (done) {
  ...
  describe('constructs a native payment transaction with integer timebounds', function () {
    it('should have have timebounds', function (done) {
      ...
      let transaction = new LantahBase.TransactionBuilder(source, {
        timebounds,
        fee: 100,
        networkPassphrase: LantahBase.Networks.TESTNET
      })
  ...
  describe('setTimeout', function () {
    it('not called', function () {
      ...
      let transactionBuilder = new LantahBase.TransactionBuilder(source, {
        fee: 100
      }).addOperation(
        LantahBase.Operation.payment({
          destination:
  ...
  describe('.buildFeeBumpTransaction', function () {
    it('builds a fee bump transaction', function (done) {
      ...
      const innerSource = LantahBase.Keypair.master(networkPassphrase);
      const innerAccount = new LantahBase.Account(
      ...
      let innerTx = new LantahBase.TransactionBuilder(innerAccount, {
        fee: '200',
      ...
      let feeSource = LantahBase.Keypair.fromSecret(
        'SB7ZMPZB3YMMK5CUWENXVLZWBK4KYX4YU5JBXQNZSK2DP2Q7V3LVTO5V'
      );
      let transaction = LantahBase.TransactionBuilder.buildFeeBumpTransaction(
        feeSource,
        '200',
        innerTx,
        networkPassphrase
      ...
      expect(transaction).to.be.an.instanceof(LantahBase.FeeBumpTransaction);

      // The fee rate for fee bump is at least the fee rate of the inner transaction
      expect(() => {
        LantahBase.TransactionBuilder.buildFeeBumpTransaction(
          feeSource,
          '100',
          innerTx,
          networkPassphrase
        );
      }).to.throw(/Invalid baseFee, it should be at least 200 ug./);

      innerTx = new LantahBase.TransactionBuilder(innerAccount, {
        fee: '80',
        networkPassphrase: networkPassphrase,
        timebounds: {
          minTime: 0,
      ...
      }).to.throw(/Invalid baseFee, it should be at least 100 ug./);
      ...
      const feeBumpTx = LantahBase.TransactionBuilder.buildFeeBumpTransaction(
        feeSource,
        '200',
        innerTx,
        networkPassphrase
      ...
      const sourceAccountEd25519 = LantahBase.Keypair.fromPublicKey(
        ...
        .xdrAccountId()
        .value();
      const v0Tx = new LantahBase.xdr.TransactionV0({
        ...
        ext: new LantahBase.xdr.TransactionV0Ext(0)
      });
      const innerV0TxEnvelope =
        new LantahBase.xdr.TransactionEnvelope.envelopeTypeTxV0(
          new LantahBase.xdr.TransactionV0Envelope({
            tx: v0Tx,
      ...
      const feeBumpV0Tx =
        LantahBase.TransactionBuilder.buildFeeBumpTransaction(
          feeSource,
          '200',
          new LantahBase.Transaction(innerV0TxEnvelope, networkPassphrase),
          networkPassphrase
        );
  ...
  describe('.fromXDR', function () {
    it('builds a fee bump transaction', function () {
      ...
      let tx = LantahBase.TransactionBuilder.fromXDR(
        xdr,
        LantahBase.Networks.TESTNET
      );
      expect(tx).to.be.an.instanceof(LantahBase.FeeBumpTransaction);
      expect(tx.toXDR()).to.equal(xdr);

      tx = LantahBase.TransactionBuilder.fromXDR(
    ...
    it('builds a transaction', function () {
      ...
      expect(tx).to.be.an.instanceof(LantahBase.Transaction);
      expect(tx.toXDR()).to.equal(xdr);
  ...
  describe('muxed account support', function () {
    ...
    const base = new LantahBase.Account(
      'GA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVSGZ',
      '1234'
    );
    const source = new LantahBase.MuxedAccount(base, '2');
    const destination = new LantahBase.MuxedAccount(base, '3').accountId();

    const PUBKEY_SRC = LantahBase.StrKey.decodeEd25519PublicKey(
      source.baseAccount().accountId()
    );
    const MUXED_SRC_ID = LantahBase.xdr.Uint64.fromString(source.id());
    const networkPassphrase = 'Standalone Network ; February 2017';
    const signer = LantahBase.Keypair.master(LantahBase.Networks.TESTNET);
    ...
    it('works with muxed accounts by default', function () {
      ...
      let builder = new LantahBase.TransactionBuilder(source, {
        fee: '100',
        timebounds: { minTime: 0, maxTime: 0 },
        memo: new LantahBase.Memo(
          LantahBase.MemoText,
      ...
      let decodedTx = LantahBase.TransactionBuilder.fromXDR(
        tx.toXDR('base64'),
        networkPassphrase
      );
    ...
    it('does not regress js-lantah-sdk#646', function () {
      expect(() => {
        LantahBase.TransactionBuilder.fromXDR(
    ...
    it('works with fee-bump transactions', function () {
      ...
      let builder = new LantahBase.TransactionBuilder(source.baseAccount(), {
        fee: '100',
        timebounds: { minTime: 0, maxTime: 0 },
        networkPassphrase: networkPassphrase
      });

      const muxed = new LantahBase.MuxedAccount.fromAddress(destination, '0');
      ...
      const feeTx = LantahBase.TransactionBuilder.buildFeeBumpTransaction(
        source.accountId(),
        '1000',
        tx,
        networkPassphrase
      ...
      expect(feeTx).to.be.an.instanceof(LantahBase.FeeBumpTransaction);
      ...
      const decodedTx = LantahBase.TransactionBuilder.fromXDR(
        feeTx.toXDR('base64'),
        networkPassphrase
      );
    ...
    it('clones existing transactions', function () {
      ...
      let builder = new LantahBase.TransactionBuilder(source, {
        fee: '100',
        timebounds: { minTime: 0, maxTime: 0 },
        memo: new LantahBase.Memo(LantahBase.MemoText, 'Testing cloning'),
        networkPassphrase
      })
      ...
      let tx = builder.build();
      let cloneTx = LantahBase.TransactionBuilder.cloneFrom(tx).build();
      ...
      cloneTx = LantahBase.TransactionBuilder.cloneFrom(tx, {
        fee: '10000'
      }).build();
