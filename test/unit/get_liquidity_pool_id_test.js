...
describe('LantahBase#getLiquidityPoolId()', function () {
  it('throws an error if the liquidity pool type is not `constant_product`', function () {
    expect(() => LantahBase.getLiquidityPoolId()).to.throw(
      /liquidityPoolType is invalid/
    );

    expect(() => LantahBase.getLiquidityPoolId(1)).to.throw(
      /liquidityPoolType is invalid/
    );

    expect(() => LantahBase.getLiquidityPoolId('random_type')).to.throw(
      /liquidityPoolType is invalid/
    );
  });
  ...
  it('throws an error if assetA is invalid', function () {
    expect(() =>
      LantahBase.getLiquidityPoolId('constant_product', {})
    ).to.throw(/assetA is invalid/);

    expect(() =>
      LantahBase.getLiquidityPoolId('constant_product', { assetA: 'random' })
    ).to.throw(/assetA is invalid/);
  });
  ...
  it('returns poolId correctly', function () {
    const poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA,
      assetB,
      fee
    });
...
describe('LantahBase#getLiquidityPoolId() mirror stellar-core getPoolID() tests', function () {
  ...
  const issuer1 = LantahBase.StrKey.encodeEd25519PublicKey(
    Buffer.from(
      '0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef',
      'hex'
    )
  );
  const issuer2 = LantahBase.StrKey.encodeEd25519PublicKey(
    Buffer.from(
  ...
  it('returns poolId correctly for native and alphaNum4 (short and full length)', function () {
    let poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset.native(),
      assetB: new LantahBase.Asset('AbC', issuer1),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
    ...
    poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset.native(),
      assetB: new LantahBase.Asset('AbCd', issuer1),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
  ...
  it('returns poolId correctly for native and alphaNum12 (short and full length)', function () {
    let poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset.native(),
      assetB: new LantahBase.Asset('AbCdEfGhIjK', issuer1),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
    ...
    poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset.native(),
      assetB: new LantahBase.Asset('AbCdEfGhIjKl', issuer1),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
  ...
  it('returns poolId correctly for alphaNum4 and alphaNum12, same code but different issuer', function () {
    let poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset('aBc', issuer1),
      assetB: new LantahBase.Asset('aBc', issuer2),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
    ...
    poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset('aBcDeFgHiJkL', issuer1),
      assetB: new LantahBase.Asset('aBcDeFgHiJkL', issuer2),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
  ...
  it('returns poolId correctly for alphaNum4 and alphaNum12 do not depend on issuer or code', function () {
    const poolId = LantahBase.getLiquidityPoolId('constant_product', {
      assetA: new LantahBase.Asset('aBc', issuer1),
      assetB: new LantahBase.Asset('aBcDeFgHiJk', issuer2),
      fee
    });
    expect(poolId.toString('hex')).to.equal(
...
describe('LantahBase#LiquidityPoolFeeV18', function () {
  it('throws an error if the input assets are invalid', function () {
    expect(LantahBase.LiquidityPoolFeeV18).to.equal(30);
  });
});
