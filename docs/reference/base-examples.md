...
is giving the new account 25 GRAM as its initial balance.
...
server.accounts()
  ...
  .call()
  .then(({ sequence }) => {
    const account = new LantahSDK.Account(source.publicKey(), sequence)
    const transaction = new LantahSDK.TransactionBuilder(account, {
      fee: LantahSDK.BASE_FEE,
      networkPassphrase: Networks.TESTNET
    })
      .addOperation(LantahSDK.Operation.createAccount({
        destination: destination.publicKey(),
        startingBalance: '25'
      }))
      .setTimeout(30)
      .build()
    transaction.sign(LantahSDK.Keypair.fromSecret(source.secret()))
...
* native `GRAM` asset (`ASSET_TYPE_NATIVE`),
...
```js
var nativeAsset = LantahSDK.Asset.native();
...
var testAsset = new LantahSDK.Asset('TEST', 'GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB');
...
var googleStockAsset = new LantahSDK.Asset('US38259P7069', 'GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB');
```


## Path payment

In the example below we're sending 1000 GRAM (at max) from `GABJLI6IVBKJ7HIC5NN7HHDCIEW3CMWQ2DWYHREQQUFWSWZ2CDAMZZX4` to
...
The [path payment](https://developers.stellar.org/docs/start/list-of-operations/#path-payment-strict-receive) will cause the destination address to get 5.5 GBP. It will cost the sender no more than 1000 GRAM. In this example there will be 3 exchanges, GRAM -> USD, USD-> EUR, EUR->GBP.

```js
var keypair = LantahSDK.Keypair.fromSecret(secretString);

var source = new LantahSDK.Account(keypair.publicKey(), "46316927324160");
var transaction = new LantahSDK.TransactionBuilder(source, {
    fee: LantahSDK.BASE_FEE,
    networkPassphrase: Networks.TESTNET
  })
  .addOperation(LantahSDK.Operation.pathPayment({
      sendAsset: LantahSDK.Asset.native(),
      sendMax: "1000",
      destination: 'GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB',
      destAsset: new LantahSDK.Asset('GBP', 'GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTG2E5EDW'),
      destAmount: "5.50",
      path: [
        new LantahSDK.Asset('USD', 'GBBM6BKZPEHWYO3E3YKREDPQXMS4VK35YLNU7NFBRI26RAN7GI5POFBB'),
        new LantahSDK.Asset('EUR', 'GDTNXRLOJD2YEBPKK7KCMR7J33AAG5VZXHAJTHIG736D6LVEFLLLKPDL')
      ]
  }))
  .setTimeout(30)
  .build();
...
```js
var rootKeypair = LantahSDK.Keypair.fromSecret("SBQWY3DNPFWGSZTFNV4WQZLBOJ2GQYLTMJSWK3TTMVQXEY3INFXGO52X")
var account = new LantahSDK.Account(rootkeypair.publicKey(), "46316927324160");

var secondaryAddress = "GC6HHHS7SH7KNUAOBKVGT2QZIQLRB5UA7QAGLA3IROWPH4TN65UKNJPK";

var transaction = new LantahSDK.TransactionBuilder(account, {
    fee: LantahSDK.BASE_FEE,
    networkPassphrase: Networks.TESTNET
  })
  .addOperation(LantahSDK.Operation.setOptions({
    signer: {
...
var secondKeypair = LantahSDK.Keypair.fromSecret("SAMZUAAPLRUH62HH3XE7NVD6ZSMTWPWGM6DS4X47HLVRHEBKP4U2H5E7");
